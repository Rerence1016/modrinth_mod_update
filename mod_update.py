...
def HTTP_code_handling(HTTP_request: int):
	if (HTTP_request == 410):
		...
		print(f"{HTTP_request}: ERROR OCCURRED!")
...
def detect(JSON_FILE_NAME):
	if f"{JSON_FILE_NAME}" not in os.listdir("."):

		# No .json detected (has to be explicitly named according to `JSON_FILE_NAME`)
		print(f"NO \"{JSON_FILE_NAME}\" DETECTED!")
		...
		# .json is detected
		print(f"Detected {JSON_FILE_NAME}!")
...
def check(JSON_FILE):
	JSON_FILE = f"{os.path.abspath(JSON_FILE)}"
	MODS_JSON = open(f"{JSON_FILE}")
	MODS_JSON = MODS_JSON.read()
	MODS_JSON = json.loads(MODS_JSON)
	...
	for mod in MODS_JSON:
		if mod == "mods_folder_path":
			print(f"\nEnd of list in {JSON_FILE}! Mod updates completed!")
			break
		else:
		...
		if True:
			mod_version = versionParse(str(mod_version))
			latest_version = versionParse(latest_version)
			# If same version
			same_version = ""
...
parse.add_argument("--detect", help="Detect mods from .json file, and create .json (if it doesn't exist)", 
				   metavar="[file].json", type=str)
parse.add_argument("--check", help="Check for mod updates and downloads from Modrinth using specified .json",
				   metavar="[file].json", type=str)
